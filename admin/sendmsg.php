...
if(isset($_POST['submit']))
{
	...
	$username = "04purnimarai@gmail.com";
	$hash = "0263f7c374a4fd0982c2d74ecad837864eca0a3dfbf57c0ff15e12a2f7e87cc9";

	// Config variables. Consult http://api.txtlocal.com/docs for more info.
	$test = "0";

	// Data for text message. This is the text message data.
	$sender = "TBR2019"; // This is who the message appears to be from.
	...
	if(isset($result))
	{
			header( "refresh:3; url=donors.php" );
...
@media only screen and (max-width: 580px) {
  ...
  <?php
   $query="select * from donor_registration where id='$_GET[id]'";
  				$r=mysqli_query($dbc, $query);
				$donor_data=mysqli_fetch_assoc($r);
	?>		
			<?php
		   ...
		   <form method="post" action="sendmsg.php">
			<div>
		      <label for="name">
		      	...
		      	<input type="text" id="name" name="name" value="<?php echo $donor_data['user_name']  ?>" placeholder="Your Name" required="required" tabindex="1" autofocus="autofocus" />
		      </label> 
			</div>
			<!-- <div>
		      <label for="hash">
			...
			<!-- <div>
		      ...
		      <label for="number">
		      	...
		      	<input type="number" id="number" name="number" value="<?php echo $donor_data['phone']  ?>" placeholder="Number" tabindex="4" required="required" />
		      </label>  
			</div> 
			<div>		          
		      <label for="message">
...
Hi <?php echo $donor_data['user_name']  ?> !!
We are organizing a blooddonation camp on:
Your tokennumber is:<?php echo $donor_data['keystringkey'];  ?> 
Your are requested to become a part of it.
TEAMBSWS
</textarea> 
		      </label>  
			</div>
